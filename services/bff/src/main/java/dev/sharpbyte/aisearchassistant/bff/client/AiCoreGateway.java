package dev.sharpbyte.aisearchassistant.bff.client;
...
import dev.sharpbyte.aisearchassistant.bff.config.AiCoreProperties;
import dev.sharpbyte.aisearchassistant.bff.client.dto.AiChatRequest;
import dev.sharpbyte.aisearchassistant.bff.client.dto.AiChatResponse;
...
import dev.sharpbyte.aisearchassistant.bff.security.UserContext;
...
public class AiCoreGateway {
    ...
    public AiIngestCatalogResponse ingestCatalog(UserContext user) {
        try {
            return aiCoreRestClient
                    .get()
                    .uri("/internal/v1/ingest/catalog")
                    .header(HDR_INTERNAL, aiCoreProperties.getInternalToken())
                    .header(HDR_USER_SUB, user.subject())
                    .header(HDR_USER_ROLES, String.join(",", user.roles()))
                    .header(HDR_LIBRARY_ACCESS, String.join(",", user.libraryAccess()))
                    .retrieve()
                    .body(AiIngestCatalogResponse.class);
        } catch (RestClientResponseException e) {
            throw new AiCoreException(e.getStatusCode().value(), e.getResponseBodyAsString(), e);
        }
    }

    public AiIngestResponse ingestRun(
            AiIngestRequest request, UserContext user, String correlationId) {
        try {
            return aiCoreRestClient
                    .post()
                    .uri("/internal/v1/ingest/run")
                    .header(HDR_CORRELATION, correlationId)
                    .header(HDR_INTERNAL, aiCoreProperties.getInternalToken())
                    .header(HDR_USER_SUB, user.subject())
                    .header(HDR_USER_ROLES, String.join(",", user.roles()))
