...
from ai_assistant.search.protocol import Retriever
from ai_assistant.search.qdrant_retriever import (
    QdrantRetriever,
    ensure_collection,
    seed_qdrant_if_empty,
)
...
async def init_retrieval(settings: Settings) -> None:
    """Configure stub or Qdrant backend, ensuring collection schema + optional demo seed data."""
    ...
    await seed_qdrant_if_empty(
        _qdrant_client,
        embedder,
        settings.vector_collection,
        settings.seed_vector_collection_on_startup,
    )
